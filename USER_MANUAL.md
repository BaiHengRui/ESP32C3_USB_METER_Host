...
> 截至版本：v2.7.0 | 适用平台：Windows 10/11
...
连接成功后，日志区域将显示 已连接到 COM端口号 @ 波特率并显示设备的固件版本号和硬件版本号。

![连接设备聚焦](imgs/连接设备聚焦.png)

- 注意！图示中的COM端口号有可能不同，请以实际为准
...
设备的 LittleFS 离线记录可通过本窗口读取、查看与导出。点击主界面「离线数据」按钮，或通过菜单栏「工具 → 离线数据查看」（快捷键 `Ctrl+D`）打开。
...
| 按钮 | 功能 |
|------|------|
| 列出条目 | 向设备发送 `export:list`，列出存储文件与 LittleFS 占用 |
| 导出并查看 | 导出当前选中条目（或按编号导出）并加载显示 |
| 清除全部 | 向设备发送 `export:erase`，清除全部离线条目（不可恢复） |
| 开始记录 | 向设备发送 `rec:start`，开始离线记录 |
| 停止记录 | 向设备发送 `rec:stop`，停止离线记录并自动刷新列表显示新生成的记录文件 |
| 设置间隔 | 设置离线记录采样间隔（1/5/10/30/60 秒），发送 `record:<秒>` |
| 设置开关 | 开启/关闭设备离线数据功能，发送 `rec:enable:1` / `rec:enable:0` |
| 清空显示 | 清空当前窗口已加载的数据 |
