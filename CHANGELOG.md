...
**在线使用说明**
- 菜单栏「帮助 → 使用说明」新增入口，在线拉取 GitHub 仓库说明书并渲染，不内置说明书内容
- 支持 Markdown 渲染：标题锚点、表格、列表、引用块、代码块、行内代码与图片
- 新增侧栏目录，支持点击平滑滚动、当前章节高亮与锚点跳转
- 拉取成功后自动缓存到本地，断网时回退显示本地缓存并在状态栏提示
- 链接协议白名单与属性转义，防止恶意 Markdown 注入

**文档**
- 使用说明书「阈值设置」同步下位机 V2.4.2：停止阈值改为「或（OR）」逻辑，新增 50ms 防抖机制说明
